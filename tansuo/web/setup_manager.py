...
- 记录本次会话的 settings_path/project_dir，供 /api/setup/events 定位
  setup_journal.jsonl（= abs_data_dir(settings, project_dir)/setup_journal.jsonl，
  与 cli.py cmd_setup 的写入位置一致——cmd_setup 的 cwd 即 project_dir）。
"""
