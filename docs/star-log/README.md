...
| 2026-08-14 | 020 | [项目管理工作流（新建/打开项目 + setup agent Web 化）的设计权衡，及落地前暴露的致命 base_dir 缺陷](2026-08-14-020-project-workflow-base-dir-tansuo-isolation.md) | `设计决策` `项目管理` `web后端` `子进程` `路径解析` | ✅ 已解决 |
...
| 2026-08-18 | 028 | [界面以 agent 为主轴：监督唤醒时间线做成默认首页 + 头部 agent 状态胶囊](2026-08-18-028-agent-timeline-as-default-home.md) | `前端` `agent` `信息架构` `journal` `事件分组` | ✅ 已解决 |
...
| 2026-08-19 | 032 | [dev.bat 不加载 .env：凭据环境变量只活在旧进程的会话环境里，给 bat 补 .env 加载又连踩三层 cmd 批处理解析坑](2026-08-19-032-dev-bat-dotenv-loading.md) | `windows` `bat` `dotenv` `环境变量` `编码` `一键启动` | ✅ 已解决 |
