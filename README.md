...
│   ├── notify.py           # webhook 通知：会话结束 / agent 降级推送（零新依赖）
...
- **webhook 通知**：调参一跑几小时，人不必守在屏幕前——会话结束（跑完 / Ctrl+C /
  预算或时间耗尽）与 agent 降级时各推送一条消息，兼容钉钉 / 飞书 / Slack 自定义
  机器人（`notify.format`），订阅事件可选（`notify.events`）。`webhook_url` 支持
  `${ENV:变量名}` 引用，避免把含 token 的机器人地址提交进仓库；留空则不发通知。
  在 Web「设置」页可配置并一键发送测试消息。**通知失败绝不影响搜索**（异常全吞，
  只记日志）。
...
每个分区的 meta.yaml 还记录创建时的**提示词版本**（`prompt_version`）。提示词改变
agent 的监督行为，但它不是训练输入——所以提示词变化**不会触发新开分区**，只会让
分区选择器给旧分区挂一枚「△ 提示词已变」琥珀标记、跨分区对比页加一列提示词版本
并在组内版本不一致时提醒"谨慎对比"：不同提示词下跑出的结果，差异未必来自参数本身。
...
（启动/停止/实时日志）、大模型 API 切换（探测→写回 settings.yaml）、
webhook 通知配置与报告查看，全部在浏览器里完成。
顶部**项目选择器**可新建/打开/切换项目（见下「项目管理」），**分区选择器**
可回看任意历史记录分区（记录永不删除；「最新分区」自动跟随
...
- **项目管理**：顶部项目选择器支持**新建 / 打开 / 切换**项目。项目 = 一个目录
  （你的训练代码与数据集原位保留），tansuo 在其中建立 `.tansuo/` 工作子目录
  （settings.yaml / search_space.yaml / data / runs），一切相对路径与子进程
  工作目录都以项目目录为基准。新建时选好主训练脚本，随后在 agent 页
  「配置 agent（setup）」页签点**开始配置**：LLM 阅读训练脚本起草配置并跑探针
  验证，实时日志与事件流直接可见（与搜索硬互斥，任一在跑时另一边会被拒绝）。
  项目注册表在 `~/.tansuo_agent/projects.json`；内置 demo 始终在列，
...
- **webhook 通知**：设置页配置通知开关、机器人格式（generic / 钉钉 / 飞书 / Slack）
  与订阅事件（会话结束 / agent 降级），一键发送测试消息验证连通。webhook_url
  留空保存 = 保持现有 `${ENV:...}` 引用；填明文会警告密钥入库风险。
...
notify:
  webhook_url: ${ENV:TANSUO_WEBHOOK:}   # 机器人地址；空=不发通知
  format: generic                # generic / dingtalk / lark / slack
...
推荐流程（Web）：界面「新建 / 打开」选中你的项目目录与主训练脚本 →
agent 页「配置 agent（setup）」点**开始配置**，配置 agent 起草配置并跑探测试验
验证契约，再人工微调。命令行等价：`python cli.py setup --train 你的脚本`。
