...
│   ├── cohort.py           # 记录分区：记录永不删除 + 双指纹自动分区 + 迁移
...
└── tests\                  # 分区 72 / 空间护栏 34 / 条件空间 30 / 协议 12 / 权限降级 21 / 运行时 23 断言
...
| 命令 | 作用 |
|---|---|
| `run` | 跑搜索。`--trials N --wake-every K --workers W --hours H --no-agent --resume --new --note --cohort ID --seed --model` |
| `runs` | 列出所有记录分区（时间/备注/双指纹/试验数/最优值/与当前指纹可比性）；`runs show ID` 看详情 |
...
**双指纹**决定续跑还是新开分区（每个分区的 meta.yaml 里都留了审计明细）：
...
（✔ 一致 / △ 代码已变 / ✘ 目标已变），`runs show ID` 查看指纹覆盖了哪些文件。
Web 界面顶部的分区选择器同样可切换回看任意历史分区；仪表盘在当前代码已变化时
...
新开的分区），仪表盘在训练代码变化时横幅提示"下次运行将自动新开分区"：
...
- **改了模型/代码，之前的调参结果会混进来吗？** 不会。代码指纹变化会自动新开
  ...
  （Optuna 加载既有 study 会静默沿用库内方向，混跑必失真）。
- **想从头开始怎么办？** `python cli.py run --new`（或 Web「新开分区」）新开一个
  分区从零开始——历史记录保留，随时可回看对比。没有"删除重来"，也不需要。
